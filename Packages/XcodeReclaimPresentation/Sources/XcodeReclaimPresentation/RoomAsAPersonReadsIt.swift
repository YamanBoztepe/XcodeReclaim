...
extension LeftoverListViewModel {
    func roomWritten(_ bytes: Int) -> String {
        guard let read = inTheUnitItsMagnitudeAsksFor(bytes) else { return "\(bytes) bytes" }

        return "\(read.tenths / tenthsPerUnit).\(read.tenths % tenthsPerUnit) \(read.unit)"
    }

    func roomRounded(_ bytes: Int) -> Int {
        guard let read = inTheUnitItsMagnitudeAsksFor(bytes) else { return bytes }
...
private extension LeftoverListViewModel {
    func inTheUnitItsMagnitudeAsksFor(_ bytes: Int) -> (tenths: Int, divisor: Int, unit: String)? {
